...
class QueryComprehensionTest extends FunSuite {
  ...
  test("Translation to SQL") {
    assertEquals(EmptyQuery.toSQL, SQLSelectEmpty)
    assertEquals(tbl1.toSQL, SQLSelectTable("tb1", RelationalScheme.make(Seq(("one", Type.string), ("two", Type.integer)))))
    assertEquals(QueryMonad.embed(tbl1).buildQuery().toSQL.toSQLText,
      ("SELECT one_0 AS one, two_0 AS two FROM (SELECT one AS one_0, two AS two_0 FROM tb1)", Seq()))
    //assertEquals(qm1.run()._2, 0)
    assertEquals(qm1.buildQuery().toSQL.toSQLText,
