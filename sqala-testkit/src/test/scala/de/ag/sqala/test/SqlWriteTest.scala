...
class SqlWriteTest extends FunSuite {
  ...
  Seq(
    ...
    (None, "SELECT id FROM tbl1",
      baseQuery.copy(attributes = Seq(Query.SelectAttribute(ExprColumn("id"), None)))),
    ...
    (None, "SELECT * FROM tbl1 ORDER BY id ASC",
      baseQuery.copy(orderBy = Seq(Query.SelectOrderBy(ExprColumn("id"), Ascending)))),
    ...
    (Some("all fields"), "SELECT id, company, COUNT(employee) AS employees FROM tbl1 WHERE (id > 12) GROUP BY company HAVING (employees < 10.2) ORDER BY company ASC",
      baseQuery.copy(
        attributes = Seq(Query.SelectAttribute(ExprColumn("id"), None),
          Query.SelectAttribute(ExprColumn("company"), None),
          Query.SelectAttribute(ExprApp(Operator.Count, Seq(ExprColumn("employee"))), Some("employees"))),
        ...
        orderBy = Seq(Query.SelectOrderBy(ExprColumn("company"), Ascending))
      )),
    (None, "SELECT 'Guns N'' Roses' FROM tbl1",
      baseQuery.copy(
      attributes = Seq(Query.SelectAttribute(ExprConst(LiteralString("Guns N' Roses")), None))
      )),
